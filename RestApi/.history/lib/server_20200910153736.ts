...
import { PORT, DEV_PORT } from "./constantss";
...
const onHttpError = (error: NodeJS.ErrnoException): void => {
	...
	switch (error.code) {
		case "EACCES":
			Utils.logger("error", `2TCORP API [onHttpError]: `, `${bind} requires elevated privileges`);
			process.exit(1);
			break;
		case "EADDRINUSE":
			Utils.logger("error", `2TCORP API [onHttpError]: `, `${bind} is already in use`);
			process.exit(1);
			break;
		default:
			throw error;
...
const onHttpsError = (error: NodeJS.ErrnoException): void => {
	...
	switch (error.code) {
		case "EACCES":
			Utils.logger("error", `2TCORP API [onHttpsError]: `, `${bind} requires elevated privileges`);
			process.exit(1);
			break;
		case "EADDRINUSE":
			Utils.logger("error", `2TCORP API [onHttpsError]: `, `${bind} is already in use`);
			process.exit(1);
			break;
		default:
			throw error;
